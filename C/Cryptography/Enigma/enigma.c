...
int main()
{
    ...
    int rotorOffset[] = {22, 15, 17}; // rotor offset configuration
    char *rotor[] = {rotor3, rotor2, rotor4, reflector}; // rotor configuration

    static char plugboard[] = {'O', 'Z', 'T', 'W', 'S', 'X', 'P', 'R', 'U', 'Y', 'Q', 'N', 'V'}; // Configure plugbord mapping
    // static char plugboard[] = {'O', 'Z', 'T', 'W', 'S', 'X', 'P', 'R', 'U', 'Y', 'Q', 'N', 'V'};

    char text[] = ""; // Text/Cipher goes here
    
    printf("%s", EnigmaCipher(text, rotorOffset, rotor, plugboard));
